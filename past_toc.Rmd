...
```{r plot-exp}
basins.exp <- wr.sf.exp %>% st_drop_geometry() %>% group_by(sea,year) %>% summarise(TOCyearly = sum(TOC.export), TOC = mean(TOC.fit), TSdep = mean(TSdep), Runoff = mean(Runoff), forest = mean(forest))
